// Admin/src/features/news/NewsView.jsx
...
export default function NewsView({ currentUser }) {
  ...
  const [formData, setFormData] = useState({
    ...
    author: currentUser?.full_name || 'Barangay Administration',
    is_important: false,
    is_emergency: false,
    is_published: true,
  ...
  const handleOpenEditModal = (item) => {
    ...
    setFormData({
      ...
      author: item.author || currentUser?.full_name || 'Barangay Administration',
      is_important: !!item.is_important,
      is_emergency: !!item.is_emergency,
      is_published: item.is_published !== false,
      target_audience: item.target_audience || 'residents',
    });
    setFormError('');
    setIsFormModalOpen(true);
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        created_by_name: currentUser?.full_name || currentUser?.email || 'Barangay Admin',
      };
  ...
  const handleTogglePublish = async (item) => {
    ...
    await loadNews();
    showToast(updated.is_published ? 'Article published to residents.' : 'Article unpublished.');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-slate-900 via-blue-950 to-indigo-950 p-6 md:p-8 rounded-2xl shadow-xl text-white border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2.5 mb-2">
            ...
            <span className="text-xs font-bold tracking-widest text-blue-400 uppercase">
              Community Broadcast & Information Center
            </span>
          </div>
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">News & Announcements Management</h1>
          <p className="text-slate-300 text-sm mt-1 max-w-2xl">
            Publish official barangay bulletins, public advisories, emergency alerts, and program schedules directly to the Resident Mobile and Web Portal.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-rose-800">Emergency Alerts</p>
            <h3 className="text-2xl font-extrabold text-rose-800 mt-1">{emergencyCount}</h3>
            <p className="text-xs text-rose-800 font-semibold mt-0.5">High-priority broadcast</p>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-emerald-800">Live in Portal</p>
            <h3 className="text-2xl font-extrabold text-emerald-800 mt-1">{publishedCount}</h3>
            <p className="text-xs text-slate-600 mt-0.5">Accessible by all residents</p>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-indigo-800">Public Advisories</p>
            <h3 className="text-2xl font-extrabold text-indigo-800 mt-1">{advisoryCount}</h3>
            <p className="text-xs text-slate-600 mt-0.5">Services & healthcare info</p>
          </div>
      ...
      <div className="bg-white p-4 md:p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search by title, keywords, location, or author..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white text-slate-900 transition-colors"
            />
          </div>
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pb-1 scrollbar-none text-xs">
          <span className="text-slate-600 font-bold uppercase tracking-wider shrink-0 mr-1 flex items-center">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center">
          ...
          <h3 className="text-base font-bold text-slate-900">No News Bulletins Found</h3>
          ...
          <button
            onClick={handleOpenCreateModal}
            className="mt-4 inline-flex items-center space-x-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg shadow-sm"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredNews.map((item) => (
            <div
              key={item.id}
              className={`bg-white rounded-2xl border transition-all duration-200 flex flex-col overflow-hidden shadow-sm hover:shadow-md ${item.is_emergency
            ...
            >
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
                  <span className="flex items-center space-x-1 font-medium text-slate-700 truncate max-w-[170px]">
                    <User className="w-3.5 h-3.5 text-slate-600 shrink-0" />
                    <span className="truncate">{item.author || 'Barangay Office'}</span>
                  </span>

                  <span className="text-[11px] text-slate-600 font-semibold uppercase">
                    Audience: {item.target_audience || 'Residents'}
                  </span>
                </div>
                ...
                <div className="pt-2 flex items-center justify-between gap-2">
                  <button
                    ...
                    className="inline-flex items-center space-x-1 px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold transition-colors cursor-pointer"
                  >
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none font-medium"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2 text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none font-medium bg-white"
                >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2 text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none font-medium resize-none"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2 text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none font-mono text-xs leading-relaxed"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 text-sm border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none font-medium text-xs"
                />
              </div>

              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">
                  Author / Issuing Committee
                </label>
                <input
                  type="text"
                  placeholder="e.g. Committee on Health & Sanitation"
